#!/usr/bin/env python3
"""Script de teste para um arquivo específico."""
import sys
import os
import requests
import json
    ...
    with open(file_path, 'rb') as f:
        ...
        while True:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"Chunk {chunk_num}: OK - Progress: {data['progress']:.1f}% ({data['chunks_received']}/{data['total_chunks']})")
            else:
...
def main():
    ...
    print("\n" + "=" * 60)
    print("VID-FINGER API - Teste com Arquivo Específico")
    print("=" * 60)
    ...
    print(f"\nArquivo salvo em: storage/original/{analysis_id}/")
